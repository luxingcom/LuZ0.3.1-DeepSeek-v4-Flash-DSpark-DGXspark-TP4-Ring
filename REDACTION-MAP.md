...
2. 已对发布副本执行全量替换并重扫验证：**工作树与已提交树敏感模式残留 = 0**（扫描类别：内网 IP / 环网子网对 / 管理网末段 / docker bridge / 主机名 / 内部用户名 / 密码 / key 前缀 / 内部路径 / api key 形态）。2026-08-24 增补：环网子网对 `10.100.x/y`、`10.100.x~y`、`10.20.x` 与裸简写 → `<RING_SUBNET>`；docker bridge 桥接网段 → `<DOCKER_IP>`；管理网末段（当前档 / 旧节点档 / `~` 范围）→ `<MGMT_OCTET>`。
