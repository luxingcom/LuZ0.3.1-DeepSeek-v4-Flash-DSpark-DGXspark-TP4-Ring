...
if [ -z "$ROLE" ]; then
  if [ "$(hostname)" = "dgxspark01" ]; then ROLE=head; else ROLE=worker; fi
fi
...
# 1. 先跑只读探针
if bash /opt/aicad-prod/scripts/healthcheck_hardened.sh --role "$ROLE" --timeout 30 --grace-sec 900; then
