...
支持的测试矩阵（用户要求 + QA 方案 §6 R1-R9）：
  1. 并发档位      : --concurrency 1,2,4,6,8,12（每波 conc 在飞 = 真并发，MC 版每线程独立连接池）
...
并发实现：requests + ThreadPoolExecutor + 每线程独立 Session/连接池（engine=threads MC，解决共享单连接被 HTTP/1.1 串行复用导致并发打不满的问题）。
dspark 指标经实测在当前生产 /metrics 可读（累计 counter，测试时随请求增长）。

用法示例：
  # 全矩阵（DE + PR，默认目录）
  python3 bench_v2.py --endpoint http://<vllm_host_ip>:8001/v1 --key <KEY> \
  ...
  # dry-run（不占 GPU）
  python3 bench_v2.py --endpoint http://<vllm_host_ip>:8001/v1 --key <KEY> --dry-run
"""
...
DEFAULT_PRECHECK_HOSTS = ["dgxspark01", "dgxspark02", "dgxspark03", "dgxspark04"]
...
def _pooled_session():
    """每线程独立 Session + 连接池，避免共享单连接被 HTTP/1.1 串行复用，实现真多连接并发。"""
...
def run_wave(stream, session, url, headers, model, target_tokens, max_tokens, conc,
    ...
    with ThreadPoolExecutor(max_workers=conc) as ex:
        def _one(session, p):
            sess = _pooled_session()   # 每线程独立连接池，真并发
            return run_one(sess, url, headers, model, p, max_tokens,
                           timeout, connect_timeout, ignore_eos)
        futures = [ex.submit(_one, session, p) for p in prompts]
...
def warmup_graphs(args, session, url, headers, model, tpu, metrics_url):
    """CUDA graph 预捕获：正式测量前，对目标 concurrency 各打波短请求，触发 vLLM 捕获
    对应 batch 尺寸的 graph（prefill 批 + decode 入口），避免运行中爬升到该 concurrency 时
    即时同步捕获造成的长时间停顿（shm_broadcast 饿窗口诱因之一）。
    只做暖机、不计入测量（结果直接丢弃）。"""
    warmup_concs = parse_list(args.warmup_conc)
    if not warmup_concs:
        return
    print(f"[warmup] 开始预捕获 CUDA graph，目标 concurrency={warmup_concs} ...", flush=True)
    for conc in warmup_concs:
        # PR 型：8192 前缀 / max_tokens=1 —— 捕获「batch=conc 的 prefill 批」入 graph
        st = SeededPromptStream(args.random_seed + 10000 + conc, tpu, task=None)
        r = run_wave(st, session, url, headers, model, 8192, PR_OUTPUT_TOKENS, conc,
                     args.request_timeout, args.connect_timeout, False)
        ok = sum(1 for x in r if x.get("ok"))
        print(f"[warmup] conc={conc} PR8192/1 : ok={ok}/{len(r)}", flush=True)
        # DE 型：512 输入 / 64 输出 —— 触发多步 decode，稳定 batch=conc 的 decode graph
        st2 = SeededPromptStream(args.random_seed + 20000 + conc, tpu, task="coding")
        r2 = run_wave(st2, session, url, headers, model, 512, 64, conc,
                      args.request_timeout, args.connect_timeout, True)
        ok2 = sum(1 for x in r2 if x.get("ok"))
        print(f"[warmup] conc={conc} DE512/64: ok={ok2}/{len(r2)}", flush=True)
        time.sleep(max(5.0, args.cooldown))          # 等捕获落定 / running 回落
    # 等 running 回落 0（capture 完全落定后再进测量）
    for _ in range(15):
        snap = read_metrics(metrics_url, headers)
        if snap is not None and snap.get("running", 1) == 0:
            break
        time.sleep(5)
    print("[warmup] 完成，running 已回落，进入正式测量。", flush=True)


def main():
    ap = argparse.ArgumentParser(description="vLLM v2 基准（DE 文本吞吐 / PR 纯 prefill / 接受率 / dspark / monitor）")
    ap.add_argument("--endpoint", required=True, help="vLLM /v1 地址，如 http://<vllm_host_ip>:8001/v1")
    ...
    ap.add_argument("--concurrency", default="1,2,4,6,8,12")
    ...
    ap.add_argument("--warmup-conc", default="", help="正式测量前，对这些 concurrency 各打一波短请求，触发 vLLM 预捕获对应 batch 的 CUDA graph（如 8,12），避免运行中即时捕获停顿")
    ap.add_argument("--warmup-only", action="store_true", help="只做 CUDA graph 预捕获（--warmup-conc）后立即退出，不跑正式测量；供容器启动后自动预热使用")
    ...
    if not args.dry_run and args.warmup_conc:
        warmup_graphs(args, session, url, headers, args.model, tpu, metrics_url)
        if args.warmup_only:
            print("[warmup-only] 预捕获完成，跳过正式测量，退出。", flush=True)
            return
