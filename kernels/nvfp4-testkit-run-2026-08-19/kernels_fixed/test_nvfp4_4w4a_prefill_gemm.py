...
def make_weights(K, N, scale, device):
    """构造 NVFP4 打包权重（E2M1 低半字节优先）+ E8M0 block scale [K//16, N//128]。"""
    ...
    # 按 (k-group16, n-block128) 组织 scale：这里按 K 方向用同一个 16 行组的 max
    exp = torch.floor(torch.log2(w_scale_blocks.clamp(min=1e-30) / 6.0)) + 127.0
    exp = exp.clamp(0, 255).to(torch.uint8)  # [N//128]
    w_scale = exp.unsqueeze(0).repeat(K // 16, 1)  # [K//16, N//128]
    w_scale_f = torch.pow(2.0, w_scale.float() - 127.0)   # [K//16, N//128]
    # 展开到 [K, N]：每 scale 覆盖 K 方向 16 行、N 方向 128 列
    w_scale_full = (w_scale_f.repeat_interleave(128, dim=1)   # [K//16, N]
                             .repeat_interleave(16, dim=0))    # [K, N]
    w_scaled = w / w_scale_full
    # 量化到最近 E2M1
