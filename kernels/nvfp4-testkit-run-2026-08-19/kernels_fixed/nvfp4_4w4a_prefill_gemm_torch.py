...
def nvfp4_4w4a_prefill_gemm(
    ...
    # Move lookup table to the same device as weights (CUDA in production)
    e2m1_table = e2m1_table.to(W_packed.device)
    ...
    W_scale_fp32 = torch.pow(2.0, W_scale.to(torch.float32) - 127.0)  # [K//16, N//128]

    # Expand weight scales to full [K, N] shape
    # Each scale covers a (16 along K, 128 along N) block
    W_scale_expanded = W_scale_fp32.repeat_interleave(16, dim=0).repeat_interleave(128, dim=1)  # [K, N]

    # Apply weight scales to dequantized weights
    W_scaled = W_fp32 * W_scale_expanded  # [K, N]

    # Quantize activations to NVFP4 E2M1 with E8M0 block scales (group size 16 along K)
    group_size = 16
    num_groups_k = K // group_size

    A_grouped = A_fp32.reshape(M, num_groups_k, group_size)  # [M, K//16, 16]

    # Compute per-group max absolute value for E8M0 scale computation
    A_abs_max = A_grouped.abs().amax(dim=2, keepdim=True)  # [M, K//16, 1]

    # Compute E8M0 scales: find exponent such that max_val / scale is in e2m1 range
    # E2M1 max value is 6.0, so scale = max_val / 6.0
    A_scale_val = A_abs_max / 6.0  # [M, K//16, 1]
    A_scale_val = torch.clamp(A_scale_val, min=1e-38)

    # Encode to E8M0 (biased exponent)
    A_exp = torch.floor(torch.log2(A_scale_val)).to(torch.int32)  # [M, K//16, 1]
    A_exp_clamped = torch.clamp(A_exp + 127, 0, 255)

    # Decode E8M0 activation scales back to float
    A_scale_fp32 = torch.pow(2.0, (A_exp_clamped - 127).to(torch.float32))  # [M, K//16, 1]

    # Quantize activations to E2M1 range
    A_normalized = A_grouped / (A_scale_fp32 + 1e-38)  # [M, K//16, 16]
    ...
    # Restore sign
    A_quantized = A_sign * A_quantized_abs  # [M, K//16, 16]

    # Rescale quantized activations back to original magnitude
    A_dequant = A_quantized * A_scale_fp32  # [M, K//16, 16]
