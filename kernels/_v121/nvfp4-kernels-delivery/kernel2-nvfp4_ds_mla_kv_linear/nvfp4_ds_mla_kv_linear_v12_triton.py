...
def _nvfp4_ds_mla_kv_linear_kernel(
    ...
    pid_g = tl.program_id(1)   # D-group index [0, 64)：0..31 = K 的 32 组，32..63 = V 的 32 组

    # v12.1 修复：K/V 各 512 元素 = 各 32 组（16 元素/组），grid 第二维必须为 64
    # （MCP v2 用 32 只覆盖 K/V 各前 16 组 → 49.3% 数据缺失，生产实测暴露）
    is_v = pid_g >= 32
    g_half = pid_g - (32 * is_v)   # 0..31
...
def nvfp4_ds_mla_kv_linear(kv: torch.Tensor) -> torch.Tensor:
    ...
    def grid(meta):
        return (triton.cdiv(T, meta['TOKENS_PER_PROG']), 64)   # v12.1：K 32 组 + V 32 组
