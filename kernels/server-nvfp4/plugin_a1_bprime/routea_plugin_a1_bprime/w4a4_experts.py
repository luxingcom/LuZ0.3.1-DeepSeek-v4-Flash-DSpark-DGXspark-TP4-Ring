...
权重派生链（Task #20 适配器, Task #21 checkpoint 级验证）:
  payload:  loader [w1(gate); w3(up)] → W4A4 需 [w3(up); w1(gate)]
            (hybrid-packed: 副本 +~33GB/rank; full 与 b′ native hybrid:
             原地交换零拷贝)
  w2:       行序无关; hybrid-packed 需副本（b12x w4a16 packed prepare 就地
            销毁原始 payload）; b′ native hybrid 零拷贝共享
...
b′ native 共享路线（A3 报告 §3 路线 b′, VLLM_MOE_W4A4_NATIVE=1, 仅 mode=1）:
  W4A16 侧放弃 super() 的破坏性 packed prepare（就地重打包 + 销毁原始）,
  改用 b12x 0.15.3 公开 API prepare_w4a16_e8m0_native_weights（非破坏,
  weight_layout="modelopt"）——W4A16 kernel 与 W4A4 wrapper 共享同一份
  32.25GiB/rank payload，hybrid 双表示零副本（weight 79.82 → ~45.3GiB 量级，
  见 A3 §1.2）。配套：
    - payload 就地半交换为 up-first（同 full 模式），native prepare 以
      w13_layout="w13"（up_gate 物理序）声明——kernel 侧施加 source_n_rotation；
    - _w13_layout() 覆写为 "w13"（plan/run 调用点与 prepared 一致）；
    - 包级 __init__.install() monkeypatch b12x
      _w4a16_weight_layout_for_source（fp4_e8m0_k32 → "modelopt"），使 plan 侧
      预编译 launch 与运行时 native prepared 一致（失配即 RuntimeError）;
    - install() 同时强制 B12X_W4A16_SMALL_M_DIRECT=0：e8m0×micro direct
      （M≤8）在 b12x 0.15.3 输出错误数值（L1 GPU 实证, 双布局皆坏）——native
      模式 M≤8 走主 GEMM（与 packed 逐位相等/孤立元素 ≤2 ULP, L1 实证）;
    - E8M0 原始 scale 在 W4A4 派生 + native prepare 完成后释放
      （packed grid 新分配 ~2.02GiB/rank 替位）。
"""
import os
...
# phase3b 工程发现 #1：非 vllm 命名空间 logger 无 handler、INFO 丢失 —— 本插件
# 改挂 vllm 命名空间（仅取证可见性，无行为影响）
    ...
    if pool_mod is not None and enabled is not None and enabled():
        ...
        with pool_mod._B12X_WRAPPER_POOL_LOCK:
            ...
            if w is None:
                ...
                logger.info(
                    "[routea_plugin_a1_bprime] B12x shared wrapper pool: created "
                    "wrapper for geometry %s (pool size=%d; cross-layer "
                    "workspace dedup active)", key,
                    len(pool_mod._B12X_WRAPPER_POOL))
            else:
                logger.info(
                    "[routea_plugin_a1_bprime] B12x shared wrapper pool: reusing "
...
class W4A4B12xExperts(B12xExperts):
    ...
    b′（VLLM_MOE_W4A4_NATIVE=1，仅 mode=1 生效）：hybrid 的 W4A16 侧切
    native（modelopt）非破坏 prepare，与 W4A4 共享同一份 payload。"""

    def __init__(self, moe_config, quant_config):
        super().__init__(moe_config, quant_config)
        self._w4a4_mode = int(os.environ.get("VLLM_MOE_W4A4", "0"))
        self._w4a4_native = os.environ.get("VLLM_MOE_W4A4_NATIVE", "0") == "1"
    ...
    def _w13_layout(self) -> str:
        # b′ native hybrid: payload 已就地半交换为 [w3(up); w1(gate)]，物理序
        # = "w13"（up_gate）——native prepare / plan / run 三处须一致（kernel 侧
        # 施加 source_n_rotation）。其余路径与基座一致（loader 序 = "w31"）。
        if self._w4a4_mode == 1 and self._w4a4_native:
            return "w13"
        return super()._w13_layout()
    ...
    def _derive_w4a4(self, layer) -> None:
        ...
        if self._w4a4_mode == 2 or (self._w4a4_mode == 1 and self._w4a4_native):
            # full / b′ native hybrid: 原地行交换 [w1;w3] -> [w3;w1]（W4A4
            # kernel up-first 约定），零拷贝。full 下不做 w4a16 prepare；
            # native hybrid 下 W4A16 走非破坏性 native prepare（payload 共享）。
            tmp = w13[:, :n].clone()
            w13[:, :n].copy_(w13[:, n:])
            w13[:, n:].copy_(tmp)
            tmps = s13[:, :n].clone()
            ...
            # hybrid(packed): W4A4 自持副本（super 的 b12x packed prepare 将
            # 就地销毁原始 payload）
        ...
        sf_mb = (self._w13_sf_store.numel() + self._w2_sf_store.numel()) / 1e6
        extra_mb = (self._w13.numel() + self._w2.numel()) / 1e6 + sf_mb
        native = self._w4a4_mode == 1 and self._w4a4_native
        own_mb = 0.0 if (self._w4a4_mode == 2 or native) else extra_mb
        logger.info(
            "[routea_plugin_a1_bprime] W4A4 ready: mode=%s native=%s min_m=%d "
            "layer tensors=%.0fMB (resident-extra=%.0fMB), wrapper cg=%s "
            "max_tokens=%d",
            self._w4a4_mode, native, self._w4a4_min_m, extra_mb, own_mb, cg,
            max_tokens)

    # ---------------- b′ native prepare ----------------
    def _prepare_native_w4a16(self, layer, params_dtype, activation) -> None:
        """非破坏性 native prepare（b12x 公开 API）：payload 原样保留（W4A16
        kernel 与 W4A4 wrapper 共享同一份），仅新分配 packed E8M0 scale grid。
        替代 super() 的破坏性 packed prepare（就地重打包 + 释放原始）。"""
        from b12x.moe.fused.w4a16.prepare import prepare_w4a16_e8m0_native_weights
        from b12x.integration.tp_moe import B12XPreparedFP4MoEWeights
        ...
        act = activation
        if act is None:
            act = getattr(self.moe_config, "activation", None)
        act_str = _b12x_activation_name(act) if act is not None else "silu"
        unit = self._unit_expert_scale(w13.device, int(w13.shape[0]))
        native = prepare_w4a16_e8m0_native_weights(
            ...
            w13_layout="w13",  # 物理序已就地半交换为 up-first
        )
        # 零拷贝契约（fail-fast：b12x 升级若破坏共享在此暴露，而非静默副本）
        assert native.weight_layout == "modelopt", native.weight_layout
        assert native.w13.data_ptr() == w13.data_ptr()
        assert native.w2.data_ptr() == w2.data_ptr()
        assert native.source_format == "fp4_e8m0_k32"
        self._prepared_fp4_moe_by_dtype[params_dtype] = B12XPreparedFP4MoEWeights(
            source_format="fp4_e8m0_k32",
            w13_layout="w13",
            w4a16=native,
        )
        self._native_prepared = native
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        ...
        if self._w4a4_mode == 2:
            ...
            return
        if self._w4a4_native:
            # b′ native hybrid: 非破坏性 native prepare 替代 super 的破坏性
            # packed prepare。E8M0 原始 scale 双侧（W4A4 E4M3 派生 + native
            # packed grid）完成后释放；payload tensor 由 self._w13/_w2 与
            # prepared.w13/w2 持有（layer 参数清空仅去引用，不释放内存）。
            device = layer.w13_weight.device
            params_dtype = getattr(self.moe_config, "in_dtype", torch.bfloat16)
            activation = getattr(layer, "activation", None)
            self._prepare_native_w4a16(layer, params_dtype, activation)
            ...
            logger.info(
                "[routea_plugin_a1_bprime] b' native W4A16 prepared: "
                "weight_layout=modelopt w13_layout=w13 (payload shared with "
                "W4A4 wrapper; packed E8M0 grid allocated, source scales "
                "released)")
            return
        # hybrid(packed): 原始 payload 交给生产 W4A16 路径（b12x 就地重打包）
        super().process_weights_after_loading(layer)
    ...
    def apply(
        ...
        if self._w4a4_debug:
            logger.info("[routea_plugin_a1_bprime] dispatch M=%d -> %s", M,
