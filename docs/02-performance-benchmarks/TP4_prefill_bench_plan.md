...
| 组 | 拓扑 | 镜像/版本 | 隔离变量 |
|---|---|---|---|
| A：TP2 基线 | 2 机（<MGMT_OCTET>+<MGMT_OCTET>） | 现有 cu130 / 0.26.1.dev0 | —（历史口径） |
| B：TP4 | 4 机（<MGMT_OCTET>/<MGMT_OCTET> 有线组网） | 同 A（固化镜像） | 仅组网+TP 并行度 |
| C：TP4+cu132（可选） | 4 机 | <MGMT_OCTET> cu132 镜像 | 仅软件栈 |
