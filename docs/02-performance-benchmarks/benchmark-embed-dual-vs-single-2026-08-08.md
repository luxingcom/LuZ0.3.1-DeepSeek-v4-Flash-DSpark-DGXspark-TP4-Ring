...
**涉及节点**：.55(node01) / .59(node01) / .58(node01 网关) / .60(node01)

---

## 📌 TL;DR（执行摘要）

- **恢复动作**：停掉 03/04 的 B 组 TP2 benchmark LLM（8001，benchmark 已完成、生产 LLM 走 .186:8001 不受影响），在 03/04 各启动 **anemll 0.2.1 embed（8022 端口，单卡，VLLM_GPU_MEMORY_UTILIZATION=0.15）**，均 /health 200 + dim 1024
- **路由恢复**：litellm 网关 embed 池收敛为 **2 deployment（.188:8022 + .189:8022）**，修复了 2 个配置缺陷（.187 api_base 注释残留 active → 偶发 500；.186 孤儿行）——修复后 0 错误
...
| 节点 | 恢复前状态 | 问题 |
|------|-----------|------|
| .55/.59 | 运行 B 组 TP2 benchmark LLM（vllm-groupb-head/worker，8001） | 统一内存占满（avail 仅 6-7G），无法共存 embed |
| .58 | litellm config embed 池指向 .188:8020/.189:8020/.186:8020/.187:8022 | 4 条 deployment 全部失效（上游无服务或端口错误） |
| .60 | 无 embed 容器 | — |

### 1.2 执行动作（时间线）

1. **停 TP2**：03 `docker stop/rm vllm-groupb-head`、04 `docker stop/rm vllm-groupb-worker`（先确认 8001 无活跃业务连接；litellm LLM 路由均指向 .186:8001 = A 组，B 组 8001 仅 benchmark 用）→ 内存释放至 avail 117G
...
3. **改网关**：litellm config.yaml embed 池 → `.188:8022 + .189:8022`（2 active），注释 .187/.186；备份 config.yaml.bak-20260808204220 / config.yaml.dual-bak
4. **修复 500**：重启后测速发现偶发 500 → 根因 `.187` deployment 的 api_base 被注释但 deployment 仍 active（litellm 随机路由到无 api_base 条目）→ 整体注释该块，0 错误
...
| 节点 | 容器 | 端口 | 状态 | 预算 |
|------|------|------|------|------|
| .55(node01) | anemll-embed-8022 | 8022 | ✅ Up / health 200 / dim 1024 | 0.15 ≈ 18G |
| .59(node01) | anemll-embed-8022 | 8022 | ✅ Up / health 200 / dim 1024 | 0.15 ≈ 18G |
| .58(node01) | litellm-proxy :4000 | — | ✅ 2 deployment 路由正常 | — |

---

## 2. 测速方法与口径

- 客户端：.58（网关本机），httpx asyncio 并发；文本 ~300 字（模拟业务负载）；每并发档 25 req/worker × 3 轮；先 warmup 1 次
- 四种模式：
  - **单机**：经 litellm（临时注释 .189，仅 .188 路由）
  - **双机**：经 litellm（.188 + .189 simple-shuffle）
  - **直连 .188 / 直连 .189**：绕过网关直连上游（理论上限对照）
...
|------|--------|--------|--------|---------|---------|
| 直连 .188:8022 | 64.7 | 15.41ms | 368.8 | 553.1 | 27.32ms |
| 直连 .189:8022 | 63.9 | 15.62ms | 358.8 | 551.4 | 26.33ms |
...
### 4.3 配置缺陷修复（本轮额外价值）
- 修复前：偶发 500（约 5%）——`.187` deployment api_base 注释残留但条目 active
...
| 4 | 确认 0.15 预算在业务峰值下与 .58 后端栈无内存互斥（avail 观察） | SRE | P3 | 观察 24h |

---

## ⚠️ 待完善 / 已知局限

- 测速客户端与网关同机（.58），网络 RTT 未计入；真实业务跨机调用延迟会略高（预估 +1-3ms）
...
- 实测数据：.58 /tmp/embed_bench_litellm.py（脚本）+ 三轮测速输出（本报告 §3 表）
- 本地副本：_archive_scratch/bench_B/embed_bench_litellm.py、embed_bench_data.py
- 配置备份：.58 /home/<USER>/litellm/config.yaml.bak-20260808204220（恢复前）、config.yaml.dual-bak（单机测试前双机态）
- 容器日志：.55/.59 anemll-embed-8022（访问计数）、.58 litellm-proxy（500 根因定位）
