...
### ⑦ 过时旧节点末段/TP2 注释需修正
- **用户裁决**：所有写成旧 TP2 拓扑（旧节点末段）的过时文件/注释都需修正。
- **处置**：调度 tech-writer 全工作区 grep 定位旧末段/TP2 陈旧引用 → 按 TP4 运行态（02/03/04 管理网末段、8191 端口）修正。典型：prometheus.yml（仍旧节点末段）、文档/注释中旧末段等。
- **状态**：待执行（tech-writer）。
...
1. **控制面（管理网镜像）**：`<NODE_IP>`（含各机末段）—— TP4 分布式 TCPStore/master 走这个（实测 rank0 serve 的 `--master-addr <NODE_IP>`）。这也是 registry（<NODE_IP>:5000）所在网段。
2. **环网数据面**：`10.100.x.x`（RoCE，如 <RING_SUBNET>）+ `10.20.0.x`（另一环线段）。
