...
| Job | 修改前 | 修改后 | 说明 |
|-----|--------|--------|------|
| **vllm**（<MGMT_OCTET>:8001 + <MGMT_OCTET> 预留） | 5s | **2s**（+timeout 2s） | 快变化：吞吐/延迟/请求数 |
...
- Prometheus：**全局采集统一 10s 生效**（用户反馈预填充呈平台状后调整：vllm 2s→10s、node/dcgm 15s→10s，8 处统一；vllm <MGMT_OCTET> up，duration 0.01s）
...
| 3 | 组 B（55+59）部署时 <MGMT_OCTET>:8001 target 自动转 up（2s 采集） | 主理人 | P1 | benchmark 后 |
