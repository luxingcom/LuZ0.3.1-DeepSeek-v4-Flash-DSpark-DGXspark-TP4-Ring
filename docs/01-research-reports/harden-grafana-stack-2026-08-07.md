...
- **清理**：.58 `/tmp` 全部 13 个临时面板 JSON（8/1-8/7）+ 工作区 `_scratch*` 清零
...
| 阻塞项数量 | 0 |
| 关键行动项 | 3 条（见行动清单） |
| 建议下一步 | 观察 24h 监控栈稳定性；按需归档 .58 ~ 目录测试残留 |
...
| Grafana 面板 v13 备份 | ✅ .58 `~/vllm-dashboard-v13-20260807.json`（15.7KB） |
...
| 日志轮转 | ✅ .58 daemon.json json-file 100m×3（既有） |
...
| 位置 | 清理内容 |
|------|---------|
| .58 /tmp | 13 个临时面板 JSON（dash_v9~v13、dash_payload、dsq、dash_check/cluster/fixed/now/realtime 等 8/1-8/7 历史残留）+ p.yml |
| 工作区 | `_scratch*` 全部清零（0 残留） |
| .58 ~/backup-images 备份 tar | 保留（vllm 21G + embed 19G，容灾资产） |

## 4. 遗留（未处理，需用户决策）

- .58 `~` 目录历史测试残留（`_smoke4000.py`、`_tessa_A_recheck_2026-08-05.txt`、`D/E/F_progress.log`、`_gsm8k_dl`、`__pycache__` 等）——属于用户数据区，按安全规则不主动删除，建议归档或确认后清理
...
| 2 | 用户确认后归档/清理 .58 ~ 目录测试残留（列出清单） | SRE | P3 | 用户决策 |
