...
**前置**：A 组（58+60）TP2 加固版编排重建完成（head <MGMT_OCTET>:8001 /health 200）
**方法**：<MGMT_OCTET> 上执行（ssh aicad-server），全部 PASS 才判验收通过；FAIL 记录定位
...
| T3 | 容器状态 | `docker ps`（<MGMT_OCTET>/<MGMT_OCTET>） | head+worker 双端 Running、Restarts=0 |
...
| H3 | vllm-cluster.service enable | `systemctl is-enabled vllm-cluster.service`（<MGMT_OCTET>/<MGMT_OCTET>） | enabled（自启动/自愈） |
