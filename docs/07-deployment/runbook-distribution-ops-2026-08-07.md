...
> 面向对象：SRE / 新节点接入人员
> 适用范围：集群内镜像分发（<MGMT_OCTET> registry）、权重分发（NFS + rsync）、磁盘清理与巡检
...
```
        ...
        │                          │                          │
   ┌────▼─────┐              ┌─────▼─────┐              ┌─────▼─────┐
   │ <MGMT_OCTET> worker│             │ <MGMT_OCTET> 新节点 │             │ <MGMT_OCTET> head   │
   │           │             │           │             │            │
   │ registry:2│             │ NFS 挂载   │             │ 备份接收端  │
   │  :5000    │◄── pull ────│ /mnt/models│             │ ~/backup-  │
   │           │  (192.168.5 │ -nfs (ro) │             │ images/    │
   │ NFS server│    <MGMT_OCTET>:5000)│           │             │            │
   ...
   RoCE 平面：<MGMT_OCTET>/<MGMT_OCTET> 10.100.136.x（TP=2 vLLM，当前停机）
   Wi-Fi 平面：<NODE_IP>/24（<MGMT_OCTET> 仅在此平面）
```

**角色划分**：
- **<MGMT_OCTET>**：镜像分发源（registry:2）、权重权威源（NFS 导出 + 本地 312G 模型）、worker。
- **<MGMT_OCTET>**：消费端。镜像按需 pull；权重 NFS 只读挂载 + 可选 rsync 落本地 <MODELS_DIR>。
- **<MGMT_OCTET>**：备份接收端（接收 <MGMT_OCTET> 导出镜像备份）。
...
```
<MGMT_OCTET> registry（/data/registry, :5000） ──docker pull──►  <MGMT_OCTET> 本地镜像存储
       33 镜像 / 27 repos                             按需拉取（已验证：redis 7.4.10 拉取→运行→清理闭环）
```

- 镜像来源：<MGMT_OCTET> 现有镜像全量推送至 registry（33 镜像 → 27 repos）。
...
```
路径 A（直接读）：<MGMT_OCTET> NFS 导出 /home/<USER>/models ──ro 挂载──► <MGMT_OCTET> /mnt/models-nfs
路径 B（落本地）：<MGMT_OCTET> NFS ──sync-model.sh rsync 增量──► <MGMT_OCTET> <MODELS_DIR>（LRU 200G 管理）
```

- 路径 A：零拷贝、省 <MGMT_OCTET> 盘；只读 + soft 挂载，客户端不 hang。
- 路径 B：适合需本地高性能读取/断网可用场景，由 sync-model.sh 管理。

### 2.3 清理流（LRU + 磁盘水位）

```
<MGMT_OCTET> <MODELS_DIR>（LRU，超 200G 触发清理）
        │ cleanup-weights.sh：按 .last-used 淘汰最久未用，keep/allowlist 豁免
        ▼
<MGMT_OCTET> 磁盘水位 disk-watch.sh：70% 告警 / 85% 严重
...
| 参数 | 说明 |
|---|---|
| `<源>` | 模型名或 <MGMT_OCTET> 上路径（默认同步源 <NODE_IP>:/home/<USER>/models 下对应目录） |
...
- 用途：登记允许在 <MGMT_OCTET> 等节点拉取/保留的镜像清单（配合清理与合规审计）。
...
### S3 Docker 栈
7. 安装 Docker 29.2.1；检查 buildkit 完整性（避免 <MGMT_OCTET> 同款损坏问题）。
...
### S5 SSH 互信
12. 生成节点密钥对；三机互信（含 <MGMT_OCTET>/<MGMT_OCTET>/<MGMT_OCTET> 及本机）。
...
- 持续断链 → 检查 Wi-Fi 链路、<MGMT_OCTET> NFS 服务（`systemctl status nfs-server`）。
...
5. 若仍满：评估扩容 / 迁移到 <MGMT_OCTET> 大盘 NFS 只读方案。

### 6.4 <MGMT_OCTET> 单点

**风险**：registry 与 NFS 权威源均在 <MGMT_OCTET>，单机故障即分发中断。

**对策**：
- 镜像备份：`docker save` vLLM+embed 等关键镜像 → <MGMT_OCTET> `~/backup-images/`（已实施，后台执行中）。
- 权重备份：<MGMT_OCTET> 模型 312G 需定期备份/校验；<MGMT_OCTET> 为备份接收端。
...
| 资源 | 位置 |
|---|---|
| registry 数据 | <MGMT_OCTET> `/data/registry` |
| registry 服务 | <MGMT_OCTET> `docker run registry:2` :5000（restart=always） |
| 权重权威源 | <MGMT_OCTET> `/home/<USER>/models`（312G） |
| NFS 挂载点 | <MGMT_OCTET> `/mnt/models-nfs`（ro） |
| 本地权重缓存 | <MGMT_OCTET> `<MODELS_DIR>`（LRU 200G） |
| 分发脚本 | `/opt/distribution/` |
| 分发日志 | `/var/log/distribution.log` |
| 备份接收 | <MGMT_OCTET> `~/backup-images/` |
