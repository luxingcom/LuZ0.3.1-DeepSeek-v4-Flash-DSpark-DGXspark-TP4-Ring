...
> **主理人汇编注（2026-08-07）**：本文由 Docu 产出，落地时按实测修正：① 权重热源实际为 `.58:/home/<USER>/models`（312G）；② 脚本实际位于 `/opt/distribution/`；③ 实测参数：sync-model 默认 bwlimit 20000 KB/s、重试 5 次（可传第 3 参调大）；④ 备份实际存 `.60:~/backup-images/`（vllm-0.2.1-v026.0.tar 21G + embed-0.1.1.tar 19G）。
...
```
                         <NODE_IP>/24（Wi-Fi 内网）
   ┌───────────────────────────────┐
   │ .58 aicad-server（大容量中心） │  RoCE <NODE_IP> + Wi-Fi
   ...
   ┌──────▼─────┐  ┌─────▼──────┐        ┌────────────────┐
   │ .55 gx10-55│  │ .59 gx10-59│        │ .60 aicad-      │
...
| 数据流 | 源 | 目标 | 通道 | 特点 |
|--------|----|------|------|------|
| 镜像流 | .58 registry :5000 | .55/.59/.60 | docker pull（按需） | 无 TLS/认证（ADR-4 内网信任）；27 repos |
| 权重流-热 | .58 /home/<USER>/models | .55/.59 <MODELS_DIR> | rsync 增量 | bwlimit 默认 20MB/s、断点续传、sha256 硬校验 |
| 权重流-冷 | .58 NFS 导出 | .55/.59 /mnt/models-nfs | NFS4 ro | 只读冷访问/校验清单读取 |
| 清理流 | .55/.59 <MODELS_DIR> | —— | cleanup-weights.sh（LRU 200G + prune until=24h） | 与 sync 共用 flock 防误删 |
| 备份流 | .58 关键镜像 | .60 ~/backup-images | docker save + scp | vllm 21G + embed 19G；sha256 对账 |
...
ssh aicad-server 'docker ps | wc -l'      # .58 容器 ≥14
```
...
```
处理：registry 容器异常 → .58 `docker restart registry`；Wi-Fi 抖动 → 重试 pull（按层续传）。
...
### 4.5 .58 单点故障
现状：registry 与 NFS 均在 .58（SPOF 已记录）。缓解：关键镜像已备份 .60（40G）；权重可 NFS 冷访问 + rsync 重拉。
恢复顺序：.58 服务恢复 → registry 自启检查 → NFS export 检查 → 各节点 mount -a。
长期：registry 高可用/备用 NFS（集群待办）。

### 4.6 备份恢复
- 镜像恢复：.60 `docker load < backup-images/*.tar` 或从 registry 重 pull。
...
| .58 容器 | ssh aicad-server 'docker ps \| wc -l' | ≥14 |
...
### 6.3 已知限制与演进
- .55/.59 仅 Wi-Fi：组网后同步源切 10.100.136.x；
- deepseek 156G 校验清单待生成；
- .58 为 registry+NFS 单点 → 规划高可用；
- 四机并发同步带宽竞争 → 大任务错峰。
