...
- 全天完成：镜像清理（~225G）、.60 对齐、embed 4 机 HA、Grafana 面板修复与优化、环境迁移评估
- **当前 2 项 P0 阻塞**：① .60 生产 LLM head 镜像被误删需重新获取（34.2G 完整版）② 口令轮换（全集群同密码）
...
### 1. 恢复 .60 生产 LLM head（vllm-envE-node）
- **背景**：今日 `docker system prune -af`（.60 对齐任务）误删 Exited 状态的 vllm-envE-node 容器及其镜像（ghcr 34.2G 完整版 0d9d37607520）
- **现状**：已从 registry 拉 21.6G 版重建 head，但**初始化反复失败**（NCCL 后卡死/循环重启，日志停在 autotune 配置，内存未加载权重）；worker（.58）healthy 等待中；**LLM 生产服务当前停机**
- **行动**：
  1. .60 重新获取原始完整版镜像：`docker pull ghcr.io/anemll/dspark-vllm-gx10:0.2.1-v026.0`（34.2G，验证 ID 是否为 0d9d37607520；ghcr 直连不通则经 dockerproxy.net 镜像源）
...
### 2. 口令轮换（全集群）
- **背景**：.55/.58/.59/.60 共用密码 <PASSWORD> + 相同 sudo（Cody 审查 #2 🔴）；管理脚本曾明文落盘（已清理）
- **行动**：
...
| NFS 收窄 | /etc/exports <NODE_IP>/24 过宽 | 收窄为具体 IP（.55/.59/.60）；评估 sec=krb5p |

### 4. embed 服务恢复策略
- **背景**：为给 LLM 腾内存，.58/.60 的 embed-qwen3-vllm 已停（restart=no）；litellm 已自动 failover 到 .55/.59（验证通过 dim=1024）
- **行动**：LLM 生产恢复后评估双机内存余量，决定 embed 是否回迁 .58/.60（.55/.59 承担 embed 亦可，作为长期拓扑）

### 5. 环境迁移决策落实（任务③结论）
- **结论**：新镜像 vllm-gb10:0.26.1-cu132 无法承载 TP=2 生产（mp executor 多机 KV broadcast 不支持 + ray 连接失败，fork 差异）；**生产保留 anemll 0.2.1**
- **行动**：
  1. 更新集群架构文档：LLM=anemll 0.2.1（.60 head/.58 worker）+ embed=vllm-gb10（.55/.58/.59/.60 按需）
  2. anemll 0.2.1 镜像**三处保留**（.58/.60/registry）作为不可再生资产，禁止再 prune 误删
  3. 跟踪 vLLM 上游 mp 多机修复或 anemll 新版本，再评估迁移

### 6. 镜像与磁盘清理收尾
- .60 对齐完成（registry pull 19.2G 版 + 旧版删除 + prune 85G）；registry GC 完成（71G→40G）
- **剩余**：.60 可再 prune（RECLAIMABLE 尚有空间）；sglang 两镜像（.58 27.3G/.60 38.7G）为视频工作流资产，**确认保留或清理**；.58/.60 旧 LLM 相关文件（patch-v026、tilelang-cache、env-e-build）保留至 LLM 稳定
...
### 8. 分发机制改进
- 分摊调优：litellm simple-shuffle 偏差（.58 偏多）→ 评估 least-busy 或 weight 配置
- embed 超长输入（>8192）行为测试：定截断策略（vLLM vs 调用侧）
- litellm 网关单点：迁移 .60 或双网关（.58/.60 各一）
- 备份 cron 化：.60 关键镜像备份（vllm+embed tar 40G）目前一次性，需周期化 + sha256 对账自动化
- deepseek 156G 校验清单：.58 生成 sha256sums.txt（约 30-60 分钟），.55/.59 补跑硬校验

### 9. 稳定性改进
- .60 dockerd 存储异常记录：load 同 tar 持续产出旧 ID（疑似 containerd 快照元数据损坏），已用 registry pull 绕过；若复发需重建 dockerd 存储
...
- 组网规划：.55/.59 有线/RoCE 后同步源切 10.100.136.x（已记录，待硬件）
...
| # | 行动 | 负责角色 | 紧急度 | 预期完成 |
|---|------|---------|--------|---------|
| 1 | 恢复 .60 LLM head（获取 ghcr 34.2G 完整版镜像重建） | Zhen | P0 | 镜像获取后 30min |
...
- Tessa（测试专家）：.55/.59 验收、条件通过（A/B/C 门禁）
